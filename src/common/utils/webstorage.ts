...
function keyEndToEndInboundGroupSession(senderKey: string, sessionId: string) {
...
function getJsonItem(store: Storage, key: string) {
  try {
 ...
 * @constructor
 * @param {Storage} webStore A web storage implementation, e.g.
 ...
 * WebStorage API.
 */
export default class WebStorageSessionStore {
  store: Storage;

  constructor(webStore: Storage) {
    this.store = webStore;
    if (
      !webStore.getItem ||
      !webStore.setItem ||
   ...
   */
  getEndToEndSessions(deviceKey: any) {
  ...
  getEndToEndInboundGroupSession(senderKey: any, sessionId: any) {
  ...
  setLocalTrustedBackupPubKey(pubkey: any) {
